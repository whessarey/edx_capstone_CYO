...
```{r, message=FALSE, warning=FALSE, include=FALSE}
# Loading of the data set "summer-products-with-rating-and-performance-2020-08.csv" from my github account:
...
**Data partition**:\
Train set for model training purposes and validation set for calculating final RMSE value are created. Whole dat set has only `r NROW(processed_data)` observations, thus validation set should consist of 20% of data to have a valid data amount to evaluate model performance.
...
The baseline model has a RMSE of `r round(RMSE_baseline,0)`. Best performing algorithm is *Random Forest* which has a RMSE of `r round(RMSE_rf, 0)` with the best tune parameter `r fit_rf$bestTune` and PC1 as expected as the most important feature:\
...
Plots reveal that the two highest units of (50,000) is best predicted by *rf*. Combining these ML algorithms could probably lead to better predictions through ensembling.\
...
Following 10 base models from caret package are used  in order to build an ensemble model:
...
The ensemble model has a RMSE of `r round(RMSE_ensemble_final, 0)` and slightly better than the RMSE of *random forest* `r round(RMSE_rf_final, 0)`.\
Overall, the RMSE of ensemble model is slightly higher than the mean of *units_sold* (=`r round(mean(train_set$units_sold), 0)`) regarding train data. In addition, the median of *units_sold* regarding train data is equal to 1,000 units, thus 50% of units_sold is under this value. In conclusion, the ensemble model prediction is, strictly spoken, not precise enough.\
As the plots above show, ensemble model underestimates the two highest values equal to 100,000 units and overestimates most of units larger than 1,000 units.
...
The final RMSE value on validation set by ensemble model is `r round(RMSE_ensemble_final, 0)` which improves the RMSE value of the baseline model about `r (1- round(RMSE_ensemble_final/RMSE_baseline,2))*100`\% with an acceptable computation time.\
