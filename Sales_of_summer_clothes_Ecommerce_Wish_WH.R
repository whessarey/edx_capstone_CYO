...
########### !!!Running this R code script may take some time (1 hour + x)!!!    ##########################################
...
# Downloading of the data set "summer-products-with-rating-and-performance-2020-08.zip" from my github account:
...
# Removing features with unique values as crawl_month, currency_buyer, and theme
# Simplifiying features as product size variation id
...
test_y_plot<-as.data.frame(test_y) %>% ggplot(aes(x="", y=test_y)) + geom_boxplot() + geom_jitter()  + ylim(0, 100000) + coord_flip()
y_hat_glm_plot<-as.data.frame(y_hat_glm) %>% ggplot(aes(x="", y=y_hat_glm)) + geom_boxplot() + geom_jitter() + ylim(0, 100000) + coord_flip()
y_hat_knn_plot<-as.data.frame(y_hat_knn) %>% ggplot(aes(x="", y=y_hat_knn)) + geom_boxplot() + geom_jitter()  + ylim(0, 100000) + coord_flip()
y_hat_rf_plot<-as.data.frame(y_hat_rf) %>% ggplot(aes(x="", y=y_hat_rf)) + geom_boxplot() + geom_jitter() + ylim(0, 100000) + coord_flip()
...
validation_y_plot<-as.data.frame(validation_y) %>% ggplot(aes(x="", y=validation_y)) + geom_boxplot() + geom_jitter()  + coord_flip() +  ylim(0, 100000)
y_hat_ensemble_plot<-as.data.frame(y_hat_ensemble_final) %>% ggplot(aes(x="", y=y_hat_ensemble_final)) + geom_boxplot() + geom_jitter()  + coord_flip() +  ylim(0, 100000) 

ggarrange(validation_y_plot, y_hat_ensemble_plot,  ncol = 1, nrow =2)

y_hat_baseline<-mean(train_y)
RMSE_baseline<-RMSE(validation_y, y_hat_baseline)
RMSE_baseline